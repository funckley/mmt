...
# def decode_notes(chunks, encoding):
#     """
#     Decode chunks of tokens into a flat list of notes.
#     Each row of the input is encoded as follows:
#         (event_type, beat, position, pitch, duration, instrument)
#     :param chunks: List of chunks, where each chunk is a list of tokenized notes.
#     :param encoding: Encoding configuration dictionary.
#     :return: List of decoded notes as (beat, position, pitch, duration, program).
#     """
#     print(f"Decoding chunks: {chunks}")  # Debugging input

#     # Ensure chunks is a list of lists
#     if not isinstance(chunks[0], list):
#         chunks = [chunks]  # Wrap in a list if it's a flat list
...
#     # Decode the chunks into a flat list of notes
#     notes = []
#     for chunk in chunks:
#         for row in chunk:
#             event_type = code_type_map[int(row[0])]
...
#                 program = instrument_program_map[instrument]
#                 notes.append((beat, position, pitch, duration, program))
#             elif event_type in ("start-of-song", "instrument", "start-of-notes"):
#                 continue
#             elif event_type == "end-of-song":
#                 break
#             else:
